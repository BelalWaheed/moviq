...
import { fetchNowAiringTV } from "../../redux/HomeSlices/nowPlayingSlice";

function SeriesCarousel() {
  const { tvList, tvIsLoading } = useSelector(
    (s) => s.nowPlayingReducer.tvShows
  );
  ...
  useEffect(() => {
    dsp(fetchNowAiringTV());
  }, []);

  if (tvIsLoading) return <MovieLoader />;
