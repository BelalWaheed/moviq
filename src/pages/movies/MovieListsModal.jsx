...
const MovieListsModal = ({ lists, onClose, onCreate, openLists, setOpenLists }) => {
    ...
    const { statusData } = useSelector(state => state.CheckItemStatusReducer);
    const { AccountInfoDetails, isLogged } = useSelector(
        state => state.AccountInfoSliceReducer
    );

    const addMediaToList = listId => {
        const currentList = userListDetails?.results.find(
            userList => userList.id == listId
        );
        ...
        dispatch(
            ...
            .then(() => {
                if (currentList) {
                    if (statusData?.item_present) {
                        Swal.fire({
                            theme: "dark",
                            icon: "error",
                            title: "Oops...",
                            text: "You have already added this movie to this list."
                        });
                        setOpenLists(false);
                    } else {
                        dispatch(
...
export default MovieListsModal;
