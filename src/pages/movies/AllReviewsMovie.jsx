...
const AllReviewsMovie = () => {
  const { MovieReviewsDetails } = useSelector(
  ...
  return (
    <section className="my-10 px-4 w-full max-w-7xl mx-auto min-h-screen bg-black pt-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full mx-auto">
        {reviews.map((review) => (
          <div
            key={review.id}
            className="bg-black rounded-2xl p-5 mx-auto w-full border border-gray-800"
          >
            {/* Header */}
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <div className="flex items-center text-sm text-gray-400">
                  ...
                  {review.author_details.rating || "_"}/10
                </div>
              </div>
            </div>
      ...
      <div className="flex justify-center items-center gap-4 mt-10">
        {/* Prev Button */}
        <button
          disabled={MovieReviewsDetails?.page <= 1}
          onClick={() => {
            dispatch(
              GetMovieReviews({
                movieId,
          ...
          className={`flex items-center justify-center w-10 h-10 rounded-full transition ${
            MovieReviewsDetails?.page <= 1
              ? "bg-neutral-800 text-gray-600 cursor-not-allowed"
              : "bg-neutral-900 text-gray-300 hover:bg-red-600 hover:text-white"
          }`}
        >
        ...
        <span className="text-white text-lg font-medium select-none">
          {MovieReviewsDetails?.page} / {MovieReviewsDetails?.total_pages}
        </span>
