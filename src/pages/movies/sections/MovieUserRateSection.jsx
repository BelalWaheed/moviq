...
import { AddMovieRating } from "../../../redux/moviesSlices/PostRequest/AddMovieRating";
import Swal from "sweetalert2";
import { RequestSingIn } from "../../../redux/AuthSlices/RequestSingIn";
import { useParams } from "react-router-dom";

const MovieUserRateSection = () => {
    ...
    const { MovieRatingLoading } = useSelector(
        state => state.MovieRatingReducer
    );
    ...
    const handleSubmit = () => {
        if (isLogged && localStorage.getItem("sessionId")) {
                ...
                if (!accountMovieStatesDetails?.rated) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            if (
                                accountMovieStatesDetails?.rated.value == value
                            ) {
                                Swal.fire({
                                    icon: "info",
                                    title: "Same rating detected!",
                                    text: "You already gave this movie the same rating before. Please choose a different score if you want to update it.",
            ...
            Swal.fire({
                ...
                text: "You need to connect your TMDB account before you can rate any movie.",
                showCancelButton: true,
...
export default MovieUserRateSection;
