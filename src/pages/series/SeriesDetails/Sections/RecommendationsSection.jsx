...
const RecommendationsSection = () => {
    const { SeriesRecommendationsDetails, SeriesRecommendationsLoading } = useSelector(
    ...
    const uniqueRecommendations = useMemo(() => {
        if (!SeriesRecommendationsDetails?.results) return [];
        const seen = new Set();
        return SeriesRecommendationsDetails.results.filter((series) => {
            if (seen.has(series.id)) return false;
            seen.add(series.id);
            return true;
        });
    }, [SeriesRecommendationsDetails?.results]);
    ...
    return (
        <section className="max-w-6xl mx-auto px-6 pb-20">
            ...
            >
                {hasFetched && SeriesRecommendationsLoading ? (
