...
const MediaSection = () => {
    ...
    useEffect(() => {
        if (seriesId) {
            ...
            dispatch(getSeriesTrailer(seriesId));
        }
    }, [dispatch, seriesId]);

    // Deduplicate videos by key
    const uniqueVideos = useMemo(() => {
        if (!seriesVideosData?.results) return [];
        const seen = new Set();
        return seriesVideosData.results.filter((vid) => {
    ...
    const uniqueBackdrops = useMemo(() => {
        if (!SeriesImagesDetails?.backdrops) return [];
        const seen = new Set();
        return SeriesImagesDetails.backdrops.filter((img) => {
    ...
    // Deduplicate logos by file_path
    const uniqueLogos = useMemo(() => {
        if (!SeriesImagesDetails?.logos) return [];
        const seen = new Set();
        return SeriesImagesDetails.logos.filter((img) => {
            if (seen.has(img.file_path)) return false;
            seen.add(img.file_path);
            return true;
        });
    ...
    const uniquePosters = useMemo(() => {
        if (!SeriesImagesDetails?.posters) return [];
        const seen = new Set();
        return SeriesImagesDetails.posters.filter((img) => {
            if (seen.has(img.file_path)) return false;
            seen.add(img.file_path);
            return true;
        });
    ...
    const totalItems = uniqueVideos.length + uniqueBackdrops.length + uniqueLogos.length + uniquePosters.length;
    ...
    return (
        ...
        >
            ...
            <div className="flex overflow-x-auto overflow-y-hidden gap-4 pb-2 scroll-indicator">
                        ...
                        {seriesVideosDataLoading ? (
                            ...
                            <div className="pb-10 px-12 mx-auto mt-10 w-full sm:w-[80%] md:w-[60%] lg:w-[50%] h-[300px] flex flex-col items-center justify-center bg-gray-900 rounded-2xl border border-gray-700 shadow-lg">
                                ...
                                <p className="text-gray-500 text-sm text-center max-w-xs">
                                    Sorry, we couldn't find videos for this show.
                                </p>
                            </div>
                        ) : (
                ...
                {activeTab === "backdrops" && (
                    <>
                        ...
                        {uniqueBackdrops.length > 10 && (
                            <button
                                onClick={() => navigate(`/series/${seriesId}/backdrops`)}
                ...
                {activeTab === "logos" && (
                    <>
                        {uniqueLogos.slice(0, 10).map((logo) => (
                            <div
                        ...
                        {uniqueLogos.length > 10 && (
                            <button
                                onClick={() => navigate(`/series/${seriesId}/logos`)}
                                className="min-w-[150px] flex flex-col justify-center items-center bg-zinc-800 rounded-2xl cursor-pointer hover:scale-105 hover:bg-red-600 transition-all duration-300">
                                <span className="text-white text-lg font-bold">
                                    +{uniqueLogos.length - 10}
                                </span>
                ...
                {activeTab === "posters" && (
                    <>
                        ...
                        {uniquePosters.length > 10 && (
                            <button
                                onClick={() => navigate(`/series/${seriesId}/posters`)}
                                className="min-w-[150px] flex flex-col justify-center items-center bg-zinc-800 rounded-2xl cursor-pointer hover:scale-105 hover:bg-red-600 transition-all duration-300">
                                <span className="text-white text-lg font-bold">
                                    +{uniquePosters.length - 10}
                                </span>
