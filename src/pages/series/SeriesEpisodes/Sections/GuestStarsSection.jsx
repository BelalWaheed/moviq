...
import { FaTheaterMasks, FaUser, FaStar } from "react-icons/fa";
...
const GuestStarsSection = () => {
    ...
    // Deduplicate guest stars by id
    const uniqueGuestStars = useMemo(() => {
        if (!SeriesEpisodesData?.guest_stars) return [];
        const seen = new Set();
        return SeriesEpisodesData.guest_stars.filter((star) => {
    ...
    if (!uniqueGuestStars.length) return null;

    return (
        <CollapsibleSection 
            title="Guest Stars" 
            icon={FaStar}
            itemCount={uniqueGuestStars.length}
        >
            <div className="flex gap-4 overflow-x-auto overflow-y-hidden scroll-indicator pb-3">
                {uniqueGuestStars.slice(0, 10).map(star => (
                    <div
                        onClick={() => {
                            dispatch(GetPersonDetails({ personId: star.id }));
                            navigate(`/person/${star.id}`);
                        }}
                        key={star.id}
                        ...
                        <div className="p-2">
                            ...
                            <div className="flex items-center gap-1 text-[11px] text-gray-400 leading-tight">
                                ...
                                <span className="whitespace-normal">
                                    {star.known_for_department || "Unknown"}
                                </span>
                            </div>
                            {/* Character */}
                            <div className="flex items-center gap-1 text-[11px] text-gray-400 leading-tight">
                ...
                {uniqueGuestStars.length > 10 && (
                    <button
                        onClick={() =>
                            navigate(`/series/${seriesId}/season/${seasonNumber}/episode/${episodeNumber}/guests`)
                        }
