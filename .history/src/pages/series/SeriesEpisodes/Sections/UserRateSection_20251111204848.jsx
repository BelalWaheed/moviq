...
import { AddSeriesEpisodesRating } from "../../../../redux/SeriesSlices/PostRequest/AddSeriesEpisodesRating";
import { RequestSingIn } from "../../../../redux/AuthSlices/RequestSingIn";
import { GetSeriesEpisodesAccountStates } from "../../../../redux/SeriesSlices/GetRequest/UserInteractions/GetSeriesEpisodesAccountStates";
...
const UserRateSection = () => {
    ...
    const { accountSeriesEpisodesStatesDetails } = useSelector(
        state => state.GetaccountSeriesEpisodesStatesReducer
    );

    const { episodesRatingLoading } = useSelector(
        state => state.episodesRatingReducer
    );
    ...
    }, [accountSeriesEpisodesStatesDetails]);

    console.log(accountSeriesEpisodesStatesDetails);
    ...
    const handleSubmit = () => {
        if (isLogged && localStorage.getItem("sessionId")) {
            if (value) {
                if (!accountSeriesEpisodesStatesDetails?.rated) {
                    dispatch(
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            if (
                                accountSeriesEpisodesStatesDetails?.rated
                                    .value == value
                            ) {
                                Swal.fire({
                                    icon: "info",
