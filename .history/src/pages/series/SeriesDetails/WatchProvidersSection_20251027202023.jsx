...
const WatchProvidersSection = () => {
    ...
    return (
        <div className="container mx-8 md:mx-auto relative w-full md:w-1/2 min-h-fit z-[1] mt-6">
            ...
            {/* Dropdown */}
            {isOpen && (
                <Card className="absolute left-0 top-full mt-2 w-full bg-[#0f0f0f] border border-gray-800 rounded-xl p-4 shadow-xl max-h-56 overflow-y-auto z-50">
                    <input
                        type="text"
                    ...
                    <div className="flex flex-col gap-2 ">
                        {filteredKeys.length > 0 ? (
                            filteredKeys.map(key => (
                                <label
                                    key={key}
                                    className="flex items-center justify-between p-2 rounded-lg hover:bg-[#1c1c1c] transition-all cursor-pointer scroll-indicator">
