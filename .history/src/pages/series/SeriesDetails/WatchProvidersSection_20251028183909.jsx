...
const WatchProvidersSection = () => {
    ...
    useEffect(() => {
        const getFullCountry = async () => {
            try {
                ...
                const targetCountry = response.data
                    .filter(x => keys.includes(x.iso2))
                    .map(x => x);

                setAllCountry(targetCountry);
            } catch (error) {}
        };
        getFullCountry();
    ...
    const filteredKeys = allCountry
        .filter(coun => {
            const isAvailable = keys.includes(coun.iso2);
                        ...
                        {filteredKeys.length > 0 ? (
                            filteredKeys.map(key => (
                                <a
                                    key={key}
                                    href={
                                        SeriesWatchProvidersDetails.results[key]
                                            ?.link || "#"
                                    }
                                    target="_blank"
                                    rel="noopener noreferrer">
                                    <label className="flex items-center justify-between p-2 rounded-lg hover:bg-[#1c1c1c] transition-all cursor-pointer ">
