...
const WatchProvidersSection = () => {
    ...
    const filteredKeys = allCountry
        .filter(coun => {
            // هل الدولة ضمن قائمة الـ keys أصلاً؟
            const isAvailable = keys.includes(coun.iso2);

            // هل البحث يطابق الاسم أو الكود؟
            const matchesSearch =
                coun.country.toLowerCase().includes(searchTerm.toLowerCase()) ||
                coun.iso2.toLowerCase().includes(searchTerm.toLowerCase());
