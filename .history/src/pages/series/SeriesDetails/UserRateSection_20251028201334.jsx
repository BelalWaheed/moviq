...
const UserRateSection = () => {
    ...
    const handleChange = e => {
        ...
        // ✅ التحقق من أن القيمة مكتوبة بصيغة صحيحة (رقمين + رقم عشري واحد كحد أقصى)
        if (/^\d{0,2}(\.\d?)?$/.test(newValue)) {
            const num = parseFloat(newValue);

            // ✅ السماح فقط بالقيم من 0 إلى 10 (بما فيهم 10.0)
            if (isNaN(num) || num <= 10) {
                setValue(newValue);
            }
        }
    ...
    const handleBlur = () => {
        let newValue = parseFloat(value.replace(",", ".")); // دعم الفاصلة كبديل للنقطة
